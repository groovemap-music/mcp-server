...
The private `groovemap-agent-tools` source revision is recorded in `uv.lock`.
Developers use their normal Git credential helper; CI uses a short-lived, read-only GitHub
App token. Do not use a cross-repository PAT.
...
See the [architecture](architecture.md), [configuration](configuration.md), and
